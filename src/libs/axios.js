 ...
 * @param {boolean} callback - Callback when function is done
 * @param {boolean} chain - Whether to resolve state, or leave loading
 */
export async function request({ context, method, url, payload, mutations, root = false, callback = null, chain = false }) {
    let resolved = true
    let result = null
